...
function budgetLabelSql(alias = 'b') {
  return `(
    CASE
      ...
      ELSE printf('%04d-%s-%s', ${alias}.year, ${alias}.sphere, COALESCE(${alias}.category_id, COALESCE(${alias}.project_id, COALESCE(${alias}.earmark_id, ''))))
    END
  )`
}
...
export function getCashCheckById(id: number): CashCheckRow | null {
  const d = getDb()
  ensureCashChecksTable(d)
  const row = d
    .prepare(
