...
import { previewFile, analyzeFile, executeFile, undoImportExecution, generateImportTemplate, generateImportTestData, findMissingCategories } from '../services/imports'
import { previewMembersXlsx, executeMembersImport, generateMembersImportTemplate, generateMembersImportTestData } from '../services/membersImport'
    ...
    ipcMain.handle('reports.export', async (_e, payload) => {
        ...
        if (parsed.format === 'PDF') {
            ...
            const detailed = d2.prepare(`
                ...
                ORDER BY month ASC
            `).all(...p2) as any[]
            const orgName = (parsed.orgName && parsed.orgName.trim()) || (getActiveOrganization()?.name || (getSetting<string>('org.name') || 'BudgetO')) as string
            const outNegative = parsed.amountMode === 'OUT_NEGATIVE'
...
<style>
    ...
    ipcMain.handle('vouchers.usedCategoryIds', async () => {
        const { getServerConfig, remoteCall } = await import('../services/apiServer')
        const cfg = getServerConfig()
        if (cfg.mode === 'client') {
            if (!cfg.serverAddress) throw new Error('Kein Server konfiguriert (Netzwerk: Client)')
            return await remoteCall(cfg.serverAddress, 'vouchers.usedCategoryIds', {})
        }
        return { ids: getUsedCategoryIds() }
    })
    ...
    ipcMain.handle('imports.analyze', async (_e, payload) => {
        ...
        const res = await analyzeFile(parsed.fileBase64, parsed.mapping as any, parsed.options as any)
    ...
    ipcMain.handle('imports.undo', async (_e, payload) => {
        const parsed = ImportUndoInput.parse(payload)
        const res = await undoImportExecution(parsed.auditId)
        return ImportUndoOutput.parse(res as any)
    })
    ...
    ipcMain.handle('imports.duplicateDetails', async (_e, payload) => {
        const { ImportDuplicateDetailsInput, ImportDuplicateDetailsOutput } = await import('./schemas')
        const parsed = ImportDuplicateDetailsInput.parse(payload)
        const { getDuplicateDetailsByKey } = await import('../services/imports')
        const res = await getDuplicateDetailsByKey(parsed.pairs)
        return ImportDuplicateDetailsOutput.parse(res as any)
    })
    ...
    ipcMain.handle('members.import.preview', async (_e, payload) => {
        const parsed = MembersImportPreviewInput.parse(payload)
        const res = await previewMembersXlsx(parsed.fileBase64)
        return MembersImportPreviewOutput.parse(res as any)
    })

    ipcMain.handle('members.import.execute', async (_e, payload) => {
        const parsed = MembersImportExecuteInput.parse(payload)
        // Safety: create a backup before importing members
        try { await backup.makeBackup('preMembersImport') } catch { /* ignore backup errors */ }
        const res = await executeMembersImport(parsed.fileBase64, parsed.mapping, parsed.options as any)
        return MembersImportExecuteOutput.parse(res as any)
    })

    ipcMain.handle('members.import.template', async (_e, payload) => {
        void MembersImportTemplateInput.parse(payload)
        const res = await generateMembersImportTemplate()
        return MembersImportTemplateOutput.parse(res as any)
    })

    ipcMain.handle('members.import.testdata', async (_e, payload) => {
        void MembersImportTestDataInput.parse(payload)
        const res = await generateMembersImportTestData()
        return MembersImportTestDataOutput.parse(res as any)
    })
    ...
    ipcMain.handle('yearEnd.preCloseCheck', async (_e, payload) => {
        const parsed = YearEndPreCloseCheckInput.parse(payload)
        const res = await yearEnd.preCloseCheck(parsed.year)
        return YearEndPreCloseCheckOutput.parse(res as any)
    })
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Organization Management (Kostenstellen)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    ipcMain.handle('organizations.resetCurrentData', async () => {
        return resetCurrentOrganizationData()
    })
