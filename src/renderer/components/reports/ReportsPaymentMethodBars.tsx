          ...
          {(() => {
            ...
            return (
              <div style={{ position: 'absolute', top: 6, left: 12, background: 'rgba(0,0,0,0.5)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 8px', display: 'flex', gap: 10, alignItems: 'center', zIndex: 10 }}>
                ...
                <span className="chip" style={{ background: '#2e7d32', color: '#fff' }}>IN {eurFmt.format(r.inGross)}</span>
                <span className="chip" style={{ background: '#c62828', color: '#fff' }}>OUT {eurFmt.format(r.outGross)}</span>
              </div>
            )
          })()}
