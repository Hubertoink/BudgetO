...
export default function ReportsInOutLines(props: { activateKey?: number; refreshKey?: number; from?: string; to?: string }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    Promise.all([
      (window as any).api?.reports.monthly?.({ from: props.from, to: props.to, type: 'IN' }),
      (window as any).api?.reports.monthly?.({ from: props.from, to: props.to, type: 'OUT' })
    ]).then(([inRes, outRes]) => {
      if (cancelled) return
