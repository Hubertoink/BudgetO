...
export default function InvoicesView() {
  ...
  async function addPayment() {
    ...
    setBusyAction(true)
    try { const res = await window.api?.invoices?.addPayment?.({ invoiceId: showPayModal.id, date: payDate, amount: amt }); if (res) { setRows(prev => prev.map(r => r.id === showPayModal.id ? { ...r, paidSum: res.paidSum ?? (r.paidSum + amt), status: res.status } : r)); setShowPayModal(null); try { window.dispatchEvent(new Event('data-changed')) } catch { }; await loadSummary() } } catch (e: any) { alert(e?.message || String(e)) } finally { setBusyAction(false) }
  }
