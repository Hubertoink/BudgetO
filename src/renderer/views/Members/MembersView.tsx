...
// Local helpers copied to keep this view self-contained
function sanitizePeriodKeyLocal(s: string, interval: 'MONTHLY'|'QUARTERLY'|'YEARLY'): string {
...
export default function MembersView() {
  ...
  const [colPrefs, setColPrefs] = useState<{ showIBAN: boolean; showContribution: boolean; showAddress: boolean; showBoardTable: boolean; showNotes: boolean }>(() => {
    try {
      ...
      if (raw) {
        ...
        return { showIBAN: parsed.showIBAN ?? true, showContribution: parsed.showContribution ?? true, showAddress: parsed.showAddress ?? false, showBoardTable: parsed.showBoardTable ?? false, showNotes: parsed.showNotes ?? false }
      }
    } catch {}
  ...
  function validateIBAN(iban?: string | null): { ok: boolean; msg?: string } {
    ...
    let mod = 0
    for (let i = 0; i < nums.length; i += 7) { const part = String(mod) + nums.slice(i, i + 7); mod = Number(BigInt(part) % 97n) }
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button className="btn primary" onClick={async () => {
                  try {
                    ...
                    if (form.mode === 'create') { await (window as any).api?.members?.create?.(payload) } else { await (window as any).api?.members?.update?.(payload) }
...
function MemberStatusButton({ memberId, name, memberNo }: { memberId: number; name: string; memberNo?: string }) {
  ...
  useEffect(() => {
    ...
    async function loadStatusAndBasics() {
      try { const s = await (window as any).api?.payments?.status?.({ memberId }); if (alive) setStatus(s || null) } catch { }
    }
    loadStatusAndBasics()
              ...
              {due.length === 0 ? (
                ...
                <>
                  ...
                  <table cellPadding={6} style={{ width: '100%', marginTop: 6 }}>
                    <thead>
                      <tr><th align="left">Periode</th><th align="right">Betrag</th><th align="left">Verknüpfen</th><th align="left">Aktion</th></tr>
                    </thead>
                    <tbody>
...
function MemberTimeline({ status, history }: { status: any; history: Array<{ periodKey: string; datePaid: string; amount: number }> }) {
  ...
  const firstDueKeyForClamp = (() => {
    if (status?.nextDue) { try { return periodKeyFromDateLocal(new Date(status.nextDue)) } catch { } }
    return null
  })()
  ...
  const firstDueKey = (() => {
    if (nextDue) {
      try { const d = new Date(nextDue); return periodKeyFromDateLocal(d) } catch { }
    }
    return currentKey
  })()
  return (
...
function PaymentsAssignModal({ onClose }: { onClose: () => void }) {
  ...
  const [periodKey, setPeriodKey] = useState<string>(() => { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}` })
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal booking-modal" onClick={e => e.stopPropagation()} style={{ display: 'grid', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {mode === 'PERIOD' ? (
            <input className="input" value={periodKey} onChange={e => setPeriodKey(sanitizePeriodKeyLocal(e.target.value, interval))} title="Periode: YYYY-MM | YYYY-Q1..Q4 | YYYY" />
          ) : (
            <>
...
function PaymentsRow({ row, onChanged }: { row: { memberId: number; name: string; memberNo?: string|null; status: string; periodKey: string; interval: 'MONTHLY'|'QUARTERLY'|'YEARLY'; amount: number; paid: number; voucherId?: number|null; verified?: number }; onChanged: () => void }) {
  ...
  useEffect(() => {
    let alive = true
    async function loadStatus() { try { const s = await (window as any).api?.payments?.status?.({ memberId: row.memberId }); if (alive) setStatusData(s || null) } catch { } }
    loadStatus()
    const onChangedEv = () => loadStatus()
    try { window.addEventListener('data-changed', onChangedEv) } catch {}
    return () => { alive = false; try { window.removeEventListener('data-changed', onChangedEv) } catch {} }
  }, [row.memberId])
  ...
  return (
    <tr>
      <td title={row.memberNo || undefined}>
        ...
        <button className="btn ghost" title="Beitragsstatus & Historie" aria-label="Beitragsstatus & Historie" onClick={() => setShowStatus(true)} style={{ marginLeft: 6, width: 24, height: 24, padding: 0, borderRadius: 6, display: 'inline-grid', placeItems: 'center', color: (statusData?.state === 'OVERDUE' ? 'var(--danger)' : statusData?.state === 'OK' ? 'var(--success)' : 'var(--text-dim)') }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13 3a9 9 0 1 0 9 9h-2a7 7 0 1 1-7-7V3zm1 5h-2v6h6v-2h-4V8z"/></svg>
        </button>
        {showStatus && (
